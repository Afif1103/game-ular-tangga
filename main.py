...
# STATE BARU UNTUK KUIS PENGURANGAN
game_state = {
    ...
    "is_awaiting_add_answer": False, "add_correct_answer": 0,
    "is_awaiting_sub_answer": False, "sub_correct_answer": 0, "snake_end_pos": 0
}
...
def update_info_text():
    if game_state["is_awaiting_add_answer"] or game_state["is_awaiting_sub_answer"]: return
...
# FUNGSI INI DIMODIFIKASI UNTUK MEMICU KUIS PENGURANGAN
def finish_move():
    ...
    if current_pos in snakes:
        # JANGAN LANGSUNG TURUN! Panggil kuis pengurangan.
        trigger_subtraction_quiz(current_pos)
    elif current_pos in ladders:
        # Tangga tetap otomatis
        game_state["positions"][player_idx] = ladders[current_pos]
        redraw_all()
        # Setelah naik tangga, langsung ganti giliran
        timer.set_timeout(lambda: next_turn(), 500)
    else:
        # Jika tidak ada ular atau tangga, cek kemenangan atau ganti giliran
...
# --- Logika Kuis (Penjumlahan & Pengurangan) ---

# FUNGSI BARU UNTUK MEMICU KUIS PENGURANGAN
def trigger_subtraction_quiz(start_pos):
    end_pos = snakes[start_pos]
    game_state["sub_correct_answer"] = start_pos - end_pos
    game_state["snake_end_pos"] = end_pos
    game_state["is_awaiting_sub_answer"] = True
    
    info_text.textContent = "Awas Ular! Jawab untuk selamat."
    quiz_question.textContent = f"Anda turun berapa kotak? ({start_pos} - {end_pos}) = ?"
    quiz_answer.value = ""
    quiz_div.style.display = "block"
    quiz_answer.focus()
    
# FUNGSI INI DI-UPDATE UNTUK MENANGANI KEDUA JENIS KUIS
def handle_submit_answer(event):
    global game_state
    try: user_answer = int(quiz_answer.value)
    except ValueError: user_answer = -1

    if game_state["is_awaiting_add_answer"]:
        if user_answer == game_state["add_correct_answer"]:
        ...
        game_state["is_awaiting_add_answer"] = False

    elif game_state["is_awaiting_sub_answer"]:
        if user_answer == game_state["sub_correct_answer"]:
            info_text.textContent = "Benar! Anda selamat dari ular!"
            quiz_div.style.display = "none"
            # Karena tidak turun, langsung ganti giliran
            timer.set_timeout(lambda: next_turn(), 1500)
        else:
            info_text.textContent = "Salah! Anda turun!"
            quiz_div.style.display = "none"
            # Pindahkan pemain ke ekor ular
            game_state["positions"][game_state["turn"]] = game_state["snake_end_pos"]
            redraw_all()
            # Setelah turun, baru ganti giliran
            timer.set_timeout(lambda: next_turn(), 1500)
        game_state["is_awaiting_sub_answer"] = False
...
def finish_roll():
    ...
    game_state["add_correct_answer"] = current_pos + rolled_value
    game_state["is_awaiting_add_answer"] = True
...
def handle_roll(event):
    if game_state["is_animating"] or game_state["winner"] is not None or game_state["is_awaiting_add_answer"] or game_state["is_awaiting_sub_answer"]: return
